...
| 架构分层 | 核心组件 | 定位与核心职责 |
|---|---|---|
| **🤖 AI 编排层** | **ChipAgent** | 智能流程编排中心。利用强约束的 Playbook 驱动下层工具体系，完成端到端的自动化 EDA 验证流。 |
| **💻 视窗交互层** | **VCtrl** | IDE 控制中枢。以 VS Code 扩展的形式，将底层高度脚本化、松散的命令行工具收敛至丰富的全景可视交互界面。 |
| **🔗 需求追踪层** | **vtrack** | 验证追踪与数据度量池。打通管理项目 Feature → VP → Case 的三层链路映射，并向外暴露缺口 API。 |
| **📋 资产定义层** | **vreg** | 全栈寄存器维护与分发端。可视化管理硬件寄存器表定义，智能防范位域冲突并横向产出 RTL、UVM 及 C 驱动代码。 |
| **🛠️ 开发脚手架层** | **vtool** | 大一统指令级设施库。内置丰富的子命令设施，覆盖验证级项目骨架生成、代码审查比对、回归用例扫描收集体系。 |
| **📊 执行调度层** | **vcm** | 核心仿真测试管控底座。向上接管单点环境仿真与底层 SLURM 节点组计算回归，调度处理多工艺 EMC 自动化构建。 |
| **📈 覆盖分析层** | **vrg** | 覆盖率解析评估枢纽。通过内置 C 层引擎直接探入 VDB 底层数据库池完成到单用例颗粒度的覆盖量剥离、增量及归因。 |
